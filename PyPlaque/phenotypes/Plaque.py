...
class Plaque:
  ...
  **Plaque** class is designed to hold a single virological plaque
  ...
  def eccentricity(self):
    """
    **eccentricity method** returns for an individual plaque object,the
    eccentricity of the plaque which is found by fitting an ellipse to the
    plaque boundary and finding the eccentricity given by sqrt(1-(b^2/a^2))
    where b is the length of the semi-minor axis and a is the length of the
    semi-major axis
    ...
    ecc = 0
    # select the first contour that has more than 5 points and fit an ellipse
    # based on that
    if len(contours) != 0:
      for i in range(len(contours)):
        if len(contours[i]) >= 5:
          ...
          if ellipse[2] == 0: #if rotation angle is 0 results are not reliable
            ecc = 0
          else:
  ...
  def roundness(self):
    """
    **roundness method** returns for an individual plaque object,the roundness
    of the plaque which is found by the following ratio given by 4 * pi * Area /
     ( Perimeter^2 ) where Area is 4 * pi * radius^2
    ...
    point2 = np.array(((self.bbox[3]+self.bbox[1])/2,(self.bbox[2]+
    self.bbox[0])/2))
