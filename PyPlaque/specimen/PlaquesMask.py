...
class PlaquesMask:
    ...
    **PlaquesMask class** designed to hold binary mask of multiple
    ...
    def get_measure(self, plaques_list):
        """
        **get_measure method** returns a list of measurements based on the input of l
        ist of plaques as a dictionary. For cumulative measurements as compared to the measure method
        under class PyPlaque.phenotypes.Plaque that gives granular measurements based on each plaque.

        _Arguments_:

        plaques_list - (list, required) a list of object of type PyPlaque.phenotypes.Plaque from which several measures can be calculated.
        """ 
        plq_area_ls = []
        plq_ecc_ls = []
        plq_round_ls = []
        ...
        for plq in plaques_list:
            ...
            centre_ls.append([(plq.bbox[3]+plq.bbox[1])/2,(plq.bbox[2]+plq.bbox[0])/2])
    ...
    def plot_centroid(self):
        """
        **plot_centroid method** plots a dotted ring around all the plaques that are found in the mask. This ring is centred at the
        centroid of all the centres of individual plaques found.
        ...
        max_margin = 0 
        radius = 0
        point3 = np.array((self.measure_dict['centroid'][0],self.measure_dict['centroid'][1]))
        # Now, loop through coord arrays, and create a circle at each x,y pair
        for plq in self.plaques_list:
            point1 = np.array((plq.bbox[3],plq.bbox[2]))
            point2 = np.array(((plq.bbox[3]+plq.bbox[1])/2,(plq.bbox[2]+plq.bbox[0])/2))
            circ = Circle(((plq.bbox[3]+plq.bbox[1])/2,(plq.bbox[2]+plq.bbox[0])/2),np.linalg.norm(point1 - point2), fill = False, color = 'white')
            if np.linalg.norm(point1-point2) > max_margin : # measuring the max distance of plaque centre from their bbox corner
                max_margin = np.linalg.norm(point1-point2)

            if np.linalg.norm(point3 - point2) > radius: # measuring the max distance of plaque centres from centroid
                radius = np.linalg.norm(point3 - point2)
            ax.add_patch(circ)

        #This gives the maximum distance of a plaque bbox corner from the centroid so that a circle can be drawn around the cluster of plaques 
        main_circle = Circle((self.measure_dict['centroid'][0],self.measure_dict['centroid'][1]),radius+max_margin, fill = False, color = 'white')
