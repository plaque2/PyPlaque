...
def get_plaque_mask(inputImage,virus_params):
  ...
  # Remove elements from the label matrix which were not present in the
  ...
  # Filter out objects with area smaller than minPlaqueArea or larger than
  # maxPlaqueArea
  ...
  labels = []
  finalPlqRegImage =np.zeros_like(labelImage)
  ...
  for idx,region in enumerate(plaqueRegionProperties):
    ...
    if idx == 0:
      ...
      globalPeakCoords = np.vstack((globalPeakCoords,
      np.array([coordinates[:, 0] + x1, coordinates[:, 1] + y1]).T))
